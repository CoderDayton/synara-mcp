...
Requires [bun](https://bun.sh/). `bun install` resolves against the
committed `bun.lock`, which CI installs with `--frozen-lockfile`, so
a dependency bump has to land that lockfile alongside `package.json`.
Note: `package.json` carries an `overrides` entry forcing
`openapi-typescript`'s TypeScript peer to the project's TypeScript 6
pin; strict peer resolution refuses to install without it.
